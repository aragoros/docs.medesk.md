...
64х-разрядные. Знать разрядность крайне важно при выборе и установки
...
MongoDB является СУБД, используемое Demoxy для хранения ПДн, и для его
нормальной работы требуется установка MongoDB версии не ниже 2.6.x Ниже
приводится описание установки MongoDB под конкретные операционные
системы:
...
``c:\PDn``. В независимости от каталогов установки MongoDB и хранения
...
5. Далее завершаем установку. По умолчанию MongoDB установится в каталог
   ...
   аналогично.)
6. В случае если не существует, создаем каталог ``c:\PDn``
...
1. Скачиваем пакет `elasticsearch <http://nodejs.org/dist/v0.10.28/node-v0.10.28-x86.msi>`__.
...
4. Запустить командную строку Windows, перейти в каталог установки
   ...
   в некоторых версиях могут не выводится сообщения
...
2. Выполняем команду ``elasticsearch.bat``. Должны получить вывод следующего
   формата:

.. figure:: img/ii_es.png
   :alt:
...
.. figure:: img/ii_12.png
   :alt:

Важно удостоверится в том, что в процессе запуска MongoDB не возникло
...
-  MongoDB пытается создать/открыть файл базы ПДн в каталоге с
   ...
   удостоверится в том, что пользователь от имени которого запускается
...
запуском следует определится с параметрами запуска Demoxy. В каталоге
...
идентификатор которой выствляется вместо ``[config]``. По умолчанию
``[config] = 'default'``. 1. ``--gui`` - эта опция, соответственно,
...
``--gui``. 1. ``--ssl`` - эта опция, соответственно, вынуждает Demoxy
запускаться и работать в соответствии с криптографическим протоколом
ssl. Сертификаты при этом должны находится в каталоге
...
В таком случае при запуске Demoxy он запустится в режиме открытого
...
1. Запускаем дистрибутив и следуем предложенным инструкциям
   ...
   3. Выбмраем каталог установки и нажимаем ``Next`` (отмечено на
...
ключ, командной строке выполните:
...
::
    ...
    Verifying password - Enter PEM pass phrase: < Поддтвердите ввод >
...
имя вашей организации на адресс ``https://some.server.domain`` в этом
...
::
    ...
    Common Name (eg, your name or your server's hostname) []: < Интернет адресс для привязки сертификата >
...
был зашифрован пароль. Возможно убрать шифрование с ключа и убрать это
...
``site.key.org``, новый же дешифрованный ключ будет храниться в файле
``site.key``.
...
втечение 365 дней, выполните следующую команду в командной строке:
...
.. |image0| image:: img/ii_3.png
.. |image1| image:: img/ii_4.png
.. |image2| image:: img/ii_5.png
.. |image3| image:: img/ii_6.png
.. |image4| image:: img/ii_7.png
.. |image5| image:: img/ii_8.png
.. |image6| image:: img/ii_9.png
.. |image7| image:: img/ii_10.png
.. |image8| image:: img/ii_11.png
.. |image9| image:: img/ii_15.png
.. |image10| image:: img/ii_13.png
.. |image11| image:: img/ii_14.png
.. |image12| image:: img/ii_16.png
.. |image13| image:: img/ii_openssl_1.png
.. |image14| image:: img/ii_openssl_2.png
.. |image15| image:: img/ii_openssl_3.png
.. |image16| image:: img/ii_openssl_4.png
.. |image17| image:: img/ii_openssl_5.png
.. |image18| image:: img/ii_openssl_6.png
.. |image19| image:: img/ii_openssl_7.png

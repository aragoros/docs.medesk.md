...
ниже 3.2.x. Ниже приводится описание установки MongoDB под конкретные
...
-  `32x разрядная
   Windows <https://fastdl.mongodb.org/win32/mongodb-win32-i386-3.2.22-signed.msi>`__
-  `64x разрядная
   Windows <https://fastdl.mongodb.org/win32/mongodb-win32-x86_64-2008plus-ssl-3.4.23-signed.msi>`__
...
5. Далее завершаем установку. По умолчанию MongoDB установится в каталог
   ``c:\Program Files\MongoDB 3.2.22 Standard``. (Скриншоты показаны для
...
1. Скачиваем пакет `elasticsearch <https://download.elastic.co/elasticsearch/elasticsearch/elasticsearch-1.7.3.zip>`__.
2. Далее следует распаковать архив (название должно быть ``elasticsearsh-<версия>``),
   будем считать далее что архив распакован в ``c:\elasticsearsh-1.7.3``.
...
1. Скачать архив с готовой для использования версией можно
   `по этой ссылке <https://s3-eu-west-1.amazonaws.com/tn-releases/dmx/1.3.0-df1bfba.zip>`__.
...
2. Запускаем командную строку Windows и переходим в каталог ``\bin``, в
   ...
   выполняем команду ``cd "c:\elasticsearsh-1.7.3\bin"``)
...
1. Запускаем командную строку Windows, и переходим в каталог ``\bin``, в
   ...
   выполняем команду ``cd "c:\Program Files\MongoDB 3.2.22 Standard\bin"``)
2. Выполняем команду:
...
1. Скачиваем дистрибутив openssl, соответствующий разрядности Вашей
   ...
   `Скачать для Windows <http://slproweb.com/products/Win32OpenSSL.html>`__
...
3. ``Error: Cannot start server on specified port. Port already in use.``
   ...
   следует его освободить или изменить значение параметра ``--port`` 
   ``run_demoxy.bat``.
...
'''''''''''''''''''''''''''''''''''
  ...
  **MongoDB**
  ...
  -  `32x разрядная (версия 3.2.22)
     Windows <https://fastdl.mongodb.org/win32/mongodb-win32-i386-3.2.22-signed.msi>`__
  -  `64x разрядная (версия 3.4.23)
     Windows <https://fastdl.mongodb.org/win32/mongodb-win32-x86_64-2008plus-ssl-3.4.23-signed.msi>`__
  ...
  **Elasticsearch**
  ...
  -  `Windows <https://download.elastic.co/elasticsearch/elasticsearch/elasticsearch-1.7.3.zip>`__

  **Demoxy**

  -  `demoxy 1.3.0 <https://s3-eu-west-1.amazonaws.com/tn-releases/dmx/1.3.0-df1bfba.zip>`__

...
- **Идентификатор Demoxy** (``dmx-id`` в дальнейшем) - уникальный
  идентификатор сервера ПДН.

  Между сервером MEDESK и клиентом будет практически в любом случае находится один
  больше Demoxy. Таким образом, так как речь идет не только о передаче запросов на
  сервер, но и работе с ПДН необходимо чтобы каждый Demoxy имел уникальный dmx-id
  для настройки безопасного соединения с другими Demoxy по аналогии с моделью
  авторизации в которой используется уникальный логин и пароль.

- **Режим аутентификации**. Demoxy можно запускать в двух режимах:
  с аутентификацией и без.

  Без аутентификации: Если Demoxy запущен в этом режиме то к Demoxy может
  подключиться любой клиент. Поэтому не рекомендуется запускать Demoxy без
  аутентификации вне защищенного контура.

  С аутентификацией: В этом случае к Demoxy невозможно будет подключиться, без
  выполнения 2х условий:

  1. Клиент, отправивший запрос на Demoxy описан в "Входящих подключениях" для
     этого Demoxy.

  1. Клиент предоставляет специальный ключ авторизации.

  Соответственно, чтобы через такой Demoxy проложить маршрут к серверу, необходимо
  у предстоящего Demoxy настроить в "Исходящих марщшрутах" Demoxy запущеный в
  режиме аутентификации и там указать специальный ключ.

  О "Входящих подключениях" и "Исходящих маршрутах" далее.

- **Входящий подключения**. Для Demoxy входящие подключения - набор ``dmx-id`` с
  описательными данными и ключами авторизации, определяющий все Demoxy,
  которые могут подключится к данному, если тот запущен в режиме авторизации.

- **Исходящие маршруты**. Для Demoxy исходящие маршруты - набор описательных
  данных и параметров подключения к другим Demoxy, в котором необходимыми
  параметрами является ключ авторизации и адрес Demoxy, к которому будет
  осуществляться подключение.

Настройка Demoxy
----------------

  Перед тем как работать с вышеописанными понятиями, разберем более подробно
  часть настройки, которая описана в разделе ``Установка`` ``->``
  ``Шаг 6. Параметры запуска Demoxy``
